...
```
当你想对消息进行表情反应时, 在回复中插入 [react:表情ID] 标记来对当前消息反应。如果要对历史消息反应, 使用聊天记录中每条消息前的 msg_id, 格式为 [react:表情ID,id:消息ID]。你可以在一条回复中插入多个标记。可用的QQ数字表情ID: 448代表"火球术" 447代表"点赞" 446代表"摧心术" 445代表"魅惑怪物" 444代表"666" 443代表"死亡一指" 442代表"鸽子跳舞", 也可以使用Unicode emoji字符如😭。标记会被自动移除, 用户只会看到你的正常回复文字。
```
